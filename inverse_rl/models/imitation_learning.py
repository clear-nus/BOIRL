...
from inverse_rl.models.architectures import feedforward_energy, relu_net, linear_net ,reward_net, sigmoid_net, neg_linear_net
...
class ImitationLearning(object, metaclass=Hyperparametrized):
    ...
    def extract_paths(paths, keys=('observations', 'actions'), stack=True):
        aaa = [t['observations'] for t in paths]
        bbb = [np.shape(t['observations']) for t in paths]
        if stack:
...
class GAN_GCL(TrajectoryIRL):
    ...
    def __init__(self, env_spec, expert_trajs=None,
                 ...
                 #discrim_arch_args={"ff_arch":linear_net},
                 discrim_arch_args=None,
                 l2_reg = 0,
                 discount = 1.0,
                 init_itrs = None,
                 score_dtau=False,
        ...
        self.set_demos(expert_trajs)
        self.best_l = len(expert_trajs[0]['actions'])
        ...
        with tf.variable_scope(name) as vs:
            ...
            with tf.variable_scope('discrim') as vs2:
                ...
                self.myweights = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=vs2.name)
    ...
    def clean_paths(self,paths,keys):
        new_paths = []
        path_lengths = np.array([np.shape(t['observations'])[0] for t in paths])
        n_paths = len(paths)
        d_states = np.shape(paths[0]['observations'])[1]
        d_action = np.shape(paths[0]['actions'])[1]
        best_l = self.best_l
        for i in range(n_paths):
            paths[i]['observations'] = np.append(paths[i]['observations'],
                                                 np.zeros((best_l - np.shape(paths[i]['observations'])[0], d_states)),
                                                 axis=0)

            paths[i]['actions'] = np.append(paths[i]['actions'],
                                                 np.zeros((best_l - np.shape(paths[i]['actions'])[0], d_action)),
                                                 axis=0)

            paths[i]['rewards'] = np.append(paths[i]['rewards'],
                                            np.zeros((best_l - np.shape(paths[i]['rewards'])[0])),
                                            axis=0)

            paths[i]['agent_infos']['log_std'] = np.append(paths[i]['agent_infos']['log_std'], np.zeros(
                (best_l - np.shape(paths[i]['agent_infos']['log_std'])[0], 1)), axis=0)

            paths[i]['agent_infos']['mean'] = np.append(paths[i]['agent_infos']['mean'], np.zeros(
                (best_l - np.shape(paths[i]['agent_infos']['mean'])[0],1)), axis=0)
    ...
    def fit(self, paths, policy=None, batch_size=32, max_itrs=100, logger=None, lr=1e-3,**kwargs):
        #paths = self.clean_paths(paths,keys = ('observations', 'actions', 'a_logprobs', 'rewards') )
        self._compute_path_probs(paths, insert=True)
        self.eval_expert_probs(self.expert_trajs, policy, insert=True)
        obs, acts, path_probs = self.extract_paths(paths, keys=('observations', 'actions', 'a_logprobs'))
        expert_obs, expert_acts, expert_probs = self.extract_paths(self.expert_trajs, keys=('observations', 'actions', 'a_logprobs'))
    ...
    def eval(self, paths, **kwargs):
        ...
        scores,myweights = tf.get_default_session().run([self.energy, self.myweights],
                                          feed_dict={self.act_t: acts, self.obs_t: obs})
        scores = -scores[:,:,0]
        return scores
    ...
    def get_weights(self):
        ...
        #scores,myweights = tf.get_default_session().run([self.energy, self.myweights],
        #                                  feed_dict={self.act_t: acts, self.obs_t: obs})
        myweights = tf.get_default_session().run(self.myweights)
        return myweights
