...
def policy_walk(env, demos, step_size, n_iter, r_max, prior, alpha, gamma, **kwargs):
    ...
    # step 1
    weights = sample_random_rewards(env.n_states, step_size, r_max)
...
def sample_random_rewards(n_states, step_size, r_max):
    ...
    """
    #rewards = np.random.uniform(low=(-2.5, -2.5, -2.5), high=(0., 0., 0.), size=(1, 3)).squeeze()
    rewards = np.random.uniform(low=(-2.5, -2.5), high=(0., 0.), size=(1, 2)).squeeze()
...
def main(args):
    ...
    sampled_rewards = bayesian_irl(env, demos, step_size=[0.05, 0.05, 0.05], n_iter=args.n_iter, r_max=args.r_max,
                                   prior=prior,
                                   alpha=args.alpha, gamma=env.discount, burn_in=args.burn_in, sample_freq=1)
    np.save("Borlange_rewards.npy", sampled_rewards)
    # plot rewards
