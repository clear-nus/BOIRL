...
class TauRBF(RBF):
    def __init__(self, input_dim, demonstrations, art_demonstrations, discount, lengthscale=None):
        ...
        #self.lengthscale.fix()
        #self.lengthscale.constrain_bounded(lower=1e-36,upper=1)

    def goal_distance(self,goal_a, goal_b):
        assert goal_a.shape == goal_b.shape
        return np.linalg.norm(goal_a - goal_b, axis=-1)

    def get_reward(self, X, obs):
        is_art = False
        if len(np.shape(obs))>2:
            is_art = True
        if is_art:
            d = np.expand_dims(np.expand_dims(X,axis=1),axis=1)-obs
            r = -1*np.linalg.norm(d, axis=3)
        else:
            d = np.expand_dims(X,axis=1)-obs
            r = -1*np.linalg.norm(d,axis=2)
        return r

    def _get_proxy(self, X, X2=None):
        # return X,X2
        l_trajectory = self.demonstrations.shape[1]
        n_trajectories = self.demonstrations.shape[0]
        n_art_trajectories = self.art_demonstrations.shape[0]
        ...
        for t in range(n_trajectories):
            Rtau_x = self.get_reward(X, self.demonstrations[t,:])
            Rarttau_x = self.get_reward(X, self.art_demonstrations[:,t,:])
            discounted_Rtau_x = np.multiply(Rtau_x, discounts_x)
            discounted_Rtau_x = np.sum(discounted_Rtau_x, axis=1, keepdims=True)
            ...
            if X2 is not None:
                Rtau_x2 = self.get_reward(X2, self.demonstrations[t,:])
                Rarttau_x2 = self.get_reward(X2, self.art_demonstrations[:,t,:])
                discounted_Rtau_x2 = np.multiply(Rtau_x2, discounts_x2)
                discounted_Rtau_x2 = np.sum(discounted_Rtau_x2, axis=1,keepdims=True)
    ...
    def _scaled_dist(self, X, X2=None):
        ...
        """
        #self.lengthscale = min(self.lengthscale,1e-6)
