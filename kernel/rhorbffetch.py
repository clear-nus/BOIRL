...
class TauRBF(RBF):
    def __init__(self, input_dim, demonstrations, art_demonstrations, discount, lengthscale=None):
        # print("Using custom kernel")
        self.input_dim = input_dim
        self.demonstrations = demonstrations
        self.art_demonstrations = art_demonstrations
    ...
    def get_reward(self, X, achieved, desired):
        is_art = False
        if len(np.shape(achieved))>2:
            is_art = True
        if is_art:
            r = np.zeros((X.shape[0],achieved.shape[0],achieved.shape[1]))
        else:
            r = np.zeros((X.shape[0],achieved.shape[0]))

        for xind in range(X.shape[0]):
            x = X[xind]
            d = self.goal_distance(achieved,desired)
            r[xind] = x[1] * (d > x[0]).astype(np.float32)
        return r

    def _get_proxy(self, X, X2=None):
        # return X,X2
        inds = np.where(self.demonstrations["indices"] == 1.)[0]
        inds = np.append(inds, self.demonstrations["indices"].shape[0]).astype(np.int)
        l_trajectory = np.amax(inds[1:] - inds[0:-1])
        n_trajectories = len(inds) - 1
        n_art_trajectories = self.art_demonstrations["observation"].shape[0]
        ...
        # Rx = 1./(1. + np.exp(-1*X[:,0:1]*(np.repeat(np.expand_dims(self.features,axis=0),len(X),axis=0) - X[:,1:2])))
        ...
        if X2 is not None:
            ...
            # Rx2 = 1./(1. + np.exp(-1*X2[:,0:1]*(np.repeat(np.expand_dims(self.features,axis=0),len(X2),axis=0) - X2[:,1:2])))
            # Rx2[:, len(self.features) - 1] = 0
        proxy_x = []
        proxy_x2 = []

        #demo_states = self.demonstrations[:, :, 0].astype(int)
        #art_demo_states = self.art_demonstrations[:, :, :, 0].astype(int)
        for t in range(n_trajectories):
            Rtau_x = self.get_reward(X, self.demonstrations["achieved_goal"][inds[t]:inds[t + 1]],
                                     self.demonstrations["desired_goal"][inds[t]:inds[t + 1]])
            Rarttau_x = self.get_reward(X, self.art_demonstrations["achieved_goal"][:, inds[t]:inds[t + 1]],
                                        self.art_demonstrations["desired_goal"][:, inds[t]:inds[t + 1]])
            discounted_Rtau_x = np.multiply(Rtau_x, discounts_x[:,0:Rtau_x.shape[1]])
            discounted_Rtau_x = np.expand_dims(np.sum(discounted_Rtau_x, axis=1), axis=1)
            discounted_Rarttau_x = np.multiply(Rarttau_x, discounts_art_x[:,:,0:Rarttau_x.shape[2]])
            ...
            if X2 is not None:
                Rtau_x2 = self.get_reward(X2, self.demonstrations["achieved_goal"][inds[t]:inds[t + 1]],
                                         self.demonstrations["desired_goal"][inds[t]:inds[t + 1]])
                Rarttau_x2 = self.get_reward(X2, self.art_demonstrations["achieved_goal"][:, inds[t]:inds[t + 1]],
                                            self.art_demonstrations["desired_goal"][:, inds[t]:inds[t + 1]])
                discounted_Rtau_x2 = np.multiply(Rtau_x2, discounts_x2[:,0:Rtau_x2.shape[1]])
                discounted_Rtau_x2 = np.expand_dims(np.sum(discounted_Rtau_x2, axis=1), axis=1)
                discounted_Rarttau_x2 = np.multiply(Rarttau_x2, discounts_art_x2[:,:,0:Rarttau_x2.shape[2]])
