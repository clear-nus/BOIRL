...
def make_env(env_id, rank=0, seed=0, log_dir=None, wrapper_class=None, weights = None):
    """
    ...
    def _init():
        ...
        if weights is None:
            ...
            env = gym.make(env_id,weights=weights)
...
def create_test_env(env_id, n_envs=1, is_atari=False,
                    ...
                    log_dir='', should_render=True, hyperparams=None, weights = None):
    """
    ...
    if is_atari:
        ...
        env = SubprocVecEnv([make_env(env_id, i, seed, log_dir, wrapper_class=env_wrapper,weights=weights) for i in range(n_envs)])
    # Pybullet envs does not follow gym.render() interface
        ...
        if use_subproc:
            env = SubprocVecEnv([make_env(env_id, 0, seed, log_dir, wrapper_class=env_wrapper,weights=weights)])
        else:
            env = DummyVecEnv([_init])
    else:
        env = DummyVecEnv([make_env(env_id, 0, seed, log_dir, wrapper_class=env_wrapper,weights=weights)])
